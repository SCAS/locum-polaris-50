...
$hoopla_sql = 'SELECT bnum, callnum, descr from locum_bib_items WHERE callnum LIKE "%hoopla%"';
...
foreach ($hoopla_bib_arr as $hoopla_bib) {
	$hoopla_bibs[$hoopla_bib['bnum']] = array('callnum' => $hoopla_bib['callnum'], 'descr' => $hoopla_bib['descr']);
}
...
foreach ($hoopla_bibs as $bnum => $bib) {
	if ( preg_match( '/audio/i', $bib['callnum'] ) || preg_match( '/audio/i', $bib['descr'] ) ) {
		$hoopla_bibs[$bnum]['mat_code'] = $codes['audio'];
		if ( preg_match( '/music/i', $bib['callnum'] ) ) { $hoopla_bibs[$bnum]['mat_code'] = $codes['music']; }
	} else if ( preg_match( '/video/i', $bib['callnum'] ) || preg_match( '/video/i', $bib['descr'] ) ) {
